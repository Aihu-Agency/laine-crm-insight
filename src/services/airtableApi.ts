...
const AIRTABLE_ARCHIVED_FIELD = 'Archived'
  ...
  async toggleCustomerArchived(id: string, archived: boolean): Promise<Customer> {
    try {
      const record: AirtableCustomer = await this.makeRequest(`/customers/${id}`, {
        method: 'PATCH',
        body: { [AIRTABLE_ARCHIVED_FIELD]: archived },
      })
      return transformAirtableCustomer(record)
    } catch (error) {
      console.error('Error toggling customer archived state:', error)
      throw error
    }
  }
  ...
  async createCustomer(customerData: Partial<Customer>): Promise<Customer & { _warnings?: string[] }> {
    try {
      ...
      const cleanFields = Object.keys(airtableFieldsBase).reduce((acc: Record<string, any>, key) => {
        ...
        // Filter out empty arrays and arrays containing only empty strings
